...
data class NewArticle(
    // 新規作成記事のタイトル
    @field:Schema(example = "new-article-title", required = true, description = "新規作成記事のタイトル")
    @field:Valid
    @field:NotBlank
    @field:Length(min = 0, max = 32)
    @field:JsonProperty("title", required = true) private val title: String? = null,
    // 新規作成記事の説明
    @field:Schema(example = "new-article-description", required = true, description = "新規作成記事の説明")
    @field:Valid
    @field:NotNull
    @field:Length(min = 0, max = 64)
    @field:JsonProperty("description", required = true) private val description: String? = null,
    // 新規作成記事の本文
    @field:Schema(example = "new-article-body", required = true, description = "新規作成記事の本文")
    @field:Valid
    @field:NotNull
